...
pub struct Launcher {
    ...
    /// Maps `TaskDefinition` ID -> `TaskRun` ID
    pub(super) run_def_map: HashMap<Uuid, Uuid>,
    ...
    pub(super) task_attempts: HashMap<Uuid, i32>,
...
impl Launcher {
        ...
        loop {
            tokio::select! {
                ...
                Some(command) = command_rx.recv() => {
                    info!("Launcher [{}]: Received command.", self.id);
    ...
    async fn handle_launcher_command(&mut self, command: LauncherCommand) -> Result<(), Error> {
        match command {
            LauncherCommand::ExecutePipeline {
                ...
                info!("Launcher [{}]: Received ExecutePipeline command.", self.id);
    ...
    pub(super) async fn launch_task_worker(
        ...
        let task_run_id = self.run_def_map.get(&task_def.id).cloned().ok_or_else(|| {
            Error::Internal(format!(
        ...
        self.state_manager
            .update_task_run_status(task_run_id, TaskRunStatus::Queued, None, None)
            .await?;
    ...
    async fn handle_worker_result(&mut self, result: WorkerResult) -> Result<(), Error> {
        ...
        self.state_manager
            .update_task_run_status(
                ...
                result.final_status,
                None,
                Some(result.message.clone().unwrap_or_default().into()),
            )
            .await?;
        ...
        if result.final_status == TaskRunStatus::Failed {
            ...
            let max_retries = task_def.max_attempts.unwrap_or(3);
